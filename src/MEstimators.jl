...
"""
struct L1L2Estimator <: MEstimator; width::Float64; end
...
"""
struct HuberEstimator <: MEstimator; width::Float64; end
...
"""
struct FairEstimator <: MEstimator; width::Float64; end
...
"""
struct CauchyEstimator <: MEstimator; width::Float64; end
...
"""
struct GemanEstimator <: MEstimator; end
...
"""
struct WelschEstimator <: MEstimator; width::Float64; end
...
"""
struct TukeyEstimator <: MEstimator; width::Float64; end
...
"""
struct MultiEstimator{Estimators<:Tuple,Ranges<:Tuple} <: MEstimator
    est::Estimators
    rng::Ranges
...
MultiEstimator(x::Pair{Est1,T1}) where {Est1<:MEstimator,T1} = MultiEstimator{Tuple{Est1},Tuple{T1}}((x.first,), (x.second,))
MultiEstimator(x1::Pair{Est1,T1},x2::Pair{Est2,T2}) where {Est1<:MEstimator,T1,Est2<:MEstimator,T2} = MultiEstimator{Tuple{Est1,Est2},Tuple{T1,T2}}((x1.first,x2.first), (x1.second,x2.second))
MultiEstimator(x1::Pair{Est1,T1},x2::Pair{Est2,T2},x3::Pair{Est3,T3}) where {Est1<:MEstimator,T1,Est2<:MEstimator,T2,Est3<:MEstimator,T3} = MultiEstimator{Tuple{Est1,Est2,Est3},Tuple{T1,T2,T3}}((x1.first,x2.first,x3.first), (x1.second,x2.second,x3.second))
MultiEstimator(x1::Pair{Est1,T1},x2::Pair{Est2,T2},x3::Pair{Est3,T3},x4::Pair{Est4,T4}) where {Est1<:MEstimator,T1,Est2<:MEstimator,T2,Est3<:MEstimator,T3,Est4<:MEstimator,T4} = MultiEstimator{Tuple{Est1,Est2,Est3,Est4},Tuple{T1,T2,T3,T4}}((x1.first,x2.first,x3.first,x4.first), (x1.second,x2.second,x3.second,x4.second))
...
"""
refit_estimator(Est::Union{T,Type{T}}, res, factor=1.0) where {T<:MEstimator} = T(factor*1.43*StatsBase.mad(res, normalize=false))
function refit_estimator(est::T, res, factor=1.0) where T<:MultiEstimator
    tmp = ntuple(i->(typeof(est.est[i])(factor*1.43*StatsBase.mad(res[est.rng[i]], normalize=false)) => est.rng[i]),length(est.est))
